...
int next_20_xk( real_t* x0,  real_t* u ,real_t* out_x)
{
	...
	for (i = 0; i < 20; i++)
	{
		// store the value out_x(row i) of out_x to imtermediate x0_inter
		for (j = 0; j < 8; ++j) { x0_inter[j] = out_x[j + i * 8]; }
		// store the value u( row i)
		for (j = 0; j < 2; ++j) { u0[j] = u[j + i * 2]; }
		// calculate x(k+1)->xi_inte
		maple_exp(x0_inter, u0, x1_inter);
		// store the xi_inter to out_x(row i+1)
		for (j = 0; j < 8; ++j) { out_x[j + (i + 1) * 8] = x1_inter[j]; }
	}
